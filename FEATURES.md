...
- [x] TCG data
  - [x] Add `CARD_RARITY` constant + `cardRaritySchema`, a `cardPropertiesSchema` (`{ rarity: cardRaritySchema }`, its own file in `libs/schemas`, exported type `CardProperties`), and `cardProperties: cardPropertiesSchema.optional()` on `mapDocSchema` (+ create / update input schemas). Every card consumer (pools, owned cards, endpoint, UI) imports `CardProperties`, never a loose `rarity` type. Schema unit tests: valid `cardProperties`, missing `cardProperties`, `cardProperties` without `rarity` rejected, invalid rarity rejected
  - [x] Add schemas for `cardPools/{rarity}`, `users/{uid}/cards/{mapId}`, and `packsStored` / `packsRefillAnchor` on the user schema (optional, `packsStored` defaults to `PACKS_MAX`, a new user starts full)
...
- [x] TCG cards collection
  - [x] Data layer for `cards`: `CARD_TYPE` constant, `cardDocSchema` (map variant only for now: `type: "map"` + `mapId`) + schema tests, `TABLES` / db-refs entry, Firestore rules + rules tests (anyone reads, admin writes, user denied on create / update / delete). Additive, nothing consumes it yet
