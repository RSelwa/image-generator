...
- **Review (1 cold reviewer, 1 round):** applied the two blocking findings: the metadata URLs are built from `PAGES.ACHIEVEMENTS`, and the one-caller `getAchievements` helper is inlined into the page. Left as is: the `nav-history-link` literal (it is in untouched `nav-user.tsx` code), and a spinner that would never stop if a session had no uid (every visitor is signed in anonymously).

## VPS deploy › Dev app port and stale checkout (outside FEATURES.md)

- **Symptom:** the `dev` pm2 app on the VPS was crash-looping (`EADDRINUSE :::3000`). pm2 started both apps with a bare `pnpm -F @repo/front start` and no ecosystem file, so `next start` took its default port 3000 in both, and prod holds it. nginx proxies `geo-gamer.net` → 3000 and `dev.geo-gamer.net` → 3001.
- **`ecosystem.config.cjs` at the repo root** declares both apps (`prod` → `PORT=3000`, `dev` → `PORT=3001`) with their `/var/www/<env>` cwd, so the port lives in git instead of in a hand-typed pm2 command. One file for both apps rather than one per checkout: each checkout carries the same file and the deploy picks its app with `--only`. `.cjs` because the root `package.json` has no `"type"`.
- **Deploy action: `pm2 startOrReload ecosystem.config.cjs --only <env> --update-env`**, then `pm2 save` so a VPS reboot resurrects the apps with the right port. `startOrReload` also covers a first deploy where the app is not registered yet.
- **Deploy action: `git fetch origin <branch>` before `git reset --hard`.** The action never fetched, so `origin/<branch>` on the VPS never moved and every "successful" deploy rebuilt the same old commit (`/var/www/dev` was at `59339c0`). The useless `git config pull.rebase false` goes, nothing pulls.
- **`ecosystem.config.cjs` added to the `front` patterns** of `ci-check-workflow-changes.mjs`, so changing it redeploys.
- **Prod** switches to the ecosystem file on its next deploy from `main`, with the port it already uses (3000).

## Front Achievements › Plug the username change event

Branch `feat/change-username-event`, PR into `develop`.

- **Sent from `ChangePseudoModal`, after a successful pseudo update only.** The update is `unwrap()`ed first; the modal closes, then the event goes out. The event mutation never throws (RTK resolves `{ error }`), so a failed call can neither fail nor roll back the pseudo change. Errors are logged in the `queryFn` and normalized with `globalErrorHandler`, like every other endpoint of the API.
- **Gated on the `Achievements` flag, not on `Credits`.** Server side the endpoint is live, but the goal is a reward users see and chase to grow the app; paying credits silently before the feature ships gives no incentive and burns the one-shot achievement (a user who renamed pre-launch could never earn it once it is visible). So the event is only sent when the flag is on. `Credits` hides the balance UI, which is not what unlocking an achievement is about. The flag is client-side, so this is a product gate, not protection (Security principles).
- **Only sent when the pseudo actually changed** (`before !== after`, an avatar-only save sends nothing) and **when `change_username` is not in the cached unlocks.** The check reads the existing `getUnlockedAchievements` query (same cache as the achievements page), subscribed in the modal and skipped when the flag is off. If the unlocks are not loaded yet at submit time, the event is sent anyway: the server's idempotent transaction answers 200 and pays nothing.
- **`sendAchievementEvent` mutation in the existing `achievementsApi`**, generic over `AchievementEvent` (the union from `@repo/schemas`), so the next achievements reuse it. `fakeBaseQuery` + `fetch` with `Authorization: Bearer <auth.currentUser.getIdToken()>`, the pattern `lobbyApi` already uses for its API routes. `API_ENDPOINTS.ACHIEVEMENT_EVENTS` added. The response is parsed with a module-private discriminated union on `unlocked` (201 `{ unlocked: true, reward }` / 200 `{ unlocked: false }`); any other status is an error.
- **Invalidates the new `UnlockedAchievements` tag on any success**, 201 and 200 alike: both mean the unlock now exists, and a 200 means the cache was stale. On error nothing is invalidated.
- **Credits are not refreshed by hand**: the session user is a live `onSnapshot` on `users/{uid}` (`redux/api/auth.ts`), so the server's `increment` reaches the session on its own. Nothing displays credits yet anyway.
- **Toast on 201** (`sonner`, already mounted): "Achievement unlocked! +N credits", `changePseudo.achievementUnlocked` in `en.json` and `fr.json` with an ICU plural. It tells the user they earned something, which is the point of the feature; no toast on 200 or on error.
- **No pure helper, no new unit test.** The send condition is three named booleans in the modal (one caller, simplicity); the rest is wiring covered by the E2E spec.
- **E2E spec `e2e/achievements/change-username-event.spec.ts`** in the existing `achievements` suite. Seeds `achievements/change_username` (admin SDK) before each test and a user with `setupUser`; opens the modal with `?new-pseudo=` (the signup redirect) and changes the pseudo through new `CHANGE_PSEUDO_INPUT` / `CHANGE_PSEUDO_SUBMIT` `data-testid`s (the label is not linked to the input, and text selectors are banned). Flag on: after the first change the unlock doc holds the reward and `credits == reward`; after a second change `credits` is unchanged. Flag off: no request reaches the endpoint, no unlock doc, `credits` stays 0. Assertions with the admin SDK.
- **Review (1 cold reviewer, 1 round):** kept the modal-closed wait in the spec's `changePseudoViaUI`, flagged as an intermediate assertion: it is the sync point that proves `updateUserDoc` resolved and `closeModal` ran, i.e. the moment the event would have been sent, which the flag-off "no request" check depends on. Kept `sendChangeUsernameEvent` as a named function in the modal (it keeps `onSubmit` readable, same file) and the `No authenticated user` throw in the `queryFn` (same pattern as `lobbyApi`).

## VPS deploy › Build timeout and memory (outside FEATURES.md)

- **Symptom:** the first deploy after the fetch fix timed out. appleboy/ssh-action kills the session after its default `command_timeout` of 10m, and a full `pnpm install` + libs + `next build` on the VPS (2 vCPU, 2 GB RAM, swap full) takes longer. The build kept running orphaned while pm2 never reloaded.
- **`command_timeout: 30m`** on the deploy step.
- **`set -e`** at the top of the remote script: a failed install or build now stops the deploy instead of reloading pm2 on a broken `.next`.
- **Dev is stopped before its build** (`pm2 stop dev`, dev only). On 2 GB, a running `next start` plus `next build` plus prod swaps the box until SSH times out and prod answers in ~20s. Dev being down for the build is acceptable; `startOrReload` brings it back. Prod is not stopped: its downtime is not acceptable, and it was never crash-looping.
- **Not done, recommended:** building on the GitHub runner and shipping the Next standalone output to the VPS would take the build off the 2 GB box entirely. It's a bigger change to the deploy, left for its own ticket.
