...
export const handleJobStart = async (socialId: string, social: SocialDoc) => {
  try {
    const needAudioExtraction = Boolean(
      social.youtubeLink && (!social.audioLink || !social.soundId),
    )
    ...
    if (needAudioExtraction) {
      ...
      if (youtubeId) {
        ...
        if (existingSoundData?.storagePath) {
          logger.info(
            `Social ${socialId} — reusing existing sound ${existingSound!.id} for youtubeId ${youtubeId}`,
          )
      ...
      console.info(
        `Social ${socialId} has YouTube link but missing audio — setting status to WAITING_AUDIO_EXTRACTION`,
      )
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Audio extraction failed with status ${res.status}: ${errorText}`,
        )
      }
      ...
      if (!soundDoc.exists || !soundData) {
        throw new Error(
          `Sound doc with ID ${soundId} not found after audio extraction`,
        )
      }

      if (!soundData.storagePath) {
        logger.warn(
          `Sound doc ${soundId} is not fully populated yet (missing storagePath). It may be a delay in the audio extraction process.`,
        )

        throw new Error(
          `Sound doc ${soundId} is missing storagePath after audio extraction`,
        )
      }
    ...
    if (needRetrieveSound) {
      logger.info(
        `Social ${socialId} has soundId but missing audioLink — retrieving sound info`,
      )
      ...
      if (!soundDoc.exists || !soundData || !soundData.storagePath) {
        throw new Error(
          `Sound doc with ID ${social.soundId} not found or not storagePath while retrieving sound info`,
        )
      }
    ...
    if (!needAudioExtraction && !needRetrieveSound) {
      const hasNoAudio =
        !social.youtubeLink && !social.soundId && !social.audioLink

      if (hasNoAudio) {
        logger.info(`Social ${socialId} has no audio — picking a random sound`)

        const soundsSnapshot = await refs[TABLES.SOUNDS]
          .where("canBeUsedInPosts", "==", true)
          .get()
        const availableSounds = soundsSnapshot.docs.filter(
        ...
        logger.warn(
          `Social ${socialId} has no audio and no available sounds — proceeding without audio`,
        )
      } else {
        logger.info(
          `Social ${socialId} has all necessary info — setting status to WAITING_CAPTURE`,
        )
      }
