...
export const handlePost = async (socialId: string, social: SocialDoc) => {
  if (!social.urlCustomizedVideoStorage) {
    throw new HttpsError(
      "failed-precondition",
      `Social doc ${socialId} is missing urlCustomizedVideoStorage`,
    )
  }
  ...
  const response = await uploadPost.upload(social.urlCustomizedVideoStorage, {
    ...
    title:
      "Geo gamer is a game where you're dropped in a random location on multiples games maps and you have to find out where you are by exploring the surroundings and looking for clues. You can play with your friends now. Share, like, follow or comment if you want to see more!",
  ...
  if (!response) {
    throw new HttpsError(
      "internal",
      `Failed to upload post for social ${socialId}, invalid response from UploadPost`,
    )
  }
  ...
  const tiktokUrl =
    platforms.find((platform) => platform?.name === PLATFORMS.TIK_TOK)?.url ||
    null
  const instagramUrl =
    platforms.find((platform) => platform?.name === PLATFORMS.INSTAGRAM)?.url ||
    null
