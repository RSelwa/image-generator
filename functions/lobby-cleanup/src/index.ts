...
export const schedule_lobby_cleanup = onSchedule(
  "every 30 minutes",
  async () => {
    ...
    const abandonedDocs = docs.filter((doc) => {
      const data = doc.data()
    ...
    logger.info(
      `[lobby-cleanup] Marked ${finishedCount} lobbies as finished, ${abandonedCount} lobbies as abandoned`,
    )
  },
)
