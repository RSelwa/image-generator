...
import {
  getBadgeTextBoolean,
  getBadgeVariantBoolean,
  getBadgeVariantSounds,
} from "@/utils/badge"
...
const SocialRow = ({
  ...
  const onCheckedChange = (value: boolean) =>
    setCheckedIds((prev) =>
      value ? [...prev, sound.id] : prev.filter((id) => id !== sound.id),
    )
  ...
  return (
    <TableRow
      ...
      data-viewed={Boolean(sound.createdAt)}
    ...
    >
      ...
      <TableCell className="font-medium">
        {getDateString(sound.createdAt?.toDate())}
      </TableCell>
    </TableRow>
  )
}
...
const Page = () => {
  ...
  const isAllChecked = sounds
    ? sounds.length > 0 &&
      sounds.every((sound) => checkedIds.includes(sound.id))
    : false
    ...
    <main className="h-full-height-admin max-h-full-height-admin p-4 space-y-4">
      <section className="flex flex-col gap-2 lg:gap-8 lg:flex-row justify-between lg:items-center">
        <h1 className="text-2xl font-bold space-x-4">
          Sounds - <span className="text-primary">{sounds?.length}</span>
        ...
        <Table noWrapper>
          ...
          <TableBody>
            {sounds.map((sound) => (
              <SocialRow
                key={sound.id}
                {...{ sound, setCheckedIds, checkedIds }}
              />
            ))}
          </TableBody>
        </Table>
