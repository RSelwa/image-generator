...
const Page = () => {
  ...
  const filteredSphericals = sphericals.filter(
    (spherical) =>
      spherical.id.includes(input) ||
      spherical.game?.title?.toLowerCase().includes(input.toLowerCase()),
  )

  const hasChecked = checkedIds.length > 0
  const isAllChecked =
    filteredSphericals.length > 0 &&
    filteredSphericals.every((s) => checkedIds.includes(s.id))
  ...
  const deleteSphericals = async () => {
    await Promise.all(
      checkedIds
        ...
        .map((spherical) =>
          deleteSpherical({ gameId: spherical.gameId, id: spherical.id }),
        ),
    )
    setCheckedIds([])
  }
    ...
    <main className="h-full-height-admin max-h-full-height-admin p-4 space-y-4">
      <section className="flex flex-col gap-2 lg:gap-4 lg:flex-row justify-between lg:items-center">
        <h1 className="text-2xl font-bold space-x-4">
          Sphericals - <span className="text-primary">{sphericals.length}</span>
          <Button
            size="icon"
        ...
        <div className="space-x-2">
          <Checkbox
            ...
            className="size-"
            checked={hideReady}
            onCheckedChange={onCheckedReadyChange}
          />
          <label
            htmlFor="hide-ready"
            className="text-sm cursor-pointer select-none"
          >
            Hide ready sphericals
          </label>
        </div>
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to delete the selected sphericals? This
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction variant="marathon-destructive" asChild>
                ...
                >
                  Yes, delete {checkedIds.length} spherical
      ...
      <ScrollArea className="h-5/6 w-full m-0" horizontal>
        <Table noWrapper>
          ...
          <TableBody>
            {filteredSphericals.map((spherical) => (
              <SphericalRow
                key={spherical.id}
                {...{ spherical, checkedIds, setCheckedIds }}
              />
            ))}
          </TableBody>
        </Table>
