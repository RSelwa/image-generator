...
const DailyChallengeRow = ({
  ...
  const onCheckedChange = (value: boolean) =>
    setCheckedDates((prev) =>
      value
        ? [...prev, challenge.date]
        : prev.filter((d) => d !== challenge.date),
    )
  ...
  return (
    <TableRow
      onClick={() => setDate(challenge.date)}
...
const Page = () => {
  ...
  const filtered = flat.filter(
    (c) =>
      c.date.includes(input) ||
      c.gameTitle?.toLowerCase().includes(input.toLowerCase()),
  )

  const isAllChecked =
    filtered.length > 0 && filtered.every((c) => checkedDates.includes(c.date))
