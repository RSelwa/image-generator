...
export const POST = async (request: Request) => {
  try {
    ...
    const userSnapshot = await refs[TABLES.USERS]
      .where("email", "==", supporter_email)
      .limit(1)
      .get()
    const userDoc = userSnapshot.docs?.[0]

    const newTier = Object.entries(MEMBERSHIPS_ID).find(
      ([_, id]) => id === payload.data.membership_level_id,
    )?.[0]
    ...
    if (!parsedTier.success) {
      console.error(
        "Unknown membership level ID:",
        payload.data.membership_level_id,
      )
    ...
    if (userSnapshot.empty || !userDoc) {
      ...
      console.info(
        `No user found with email ${supporter_email}, coupon created`,
      )
    ...
    await refs[TABLES.USERS]
      .doc(userDoc.id)
      .update({ donorTier: parsedTier.data })

    console.info(
      `User ${userDoc.id} updated with new donor tier: ${parsedTier.data}`,
    )
