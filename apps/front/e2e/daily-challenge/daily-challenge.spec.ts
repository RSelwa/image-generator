...
test.describe("daily challenge path", () => {
  test("should display challenges and verify completion flow", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByTestId(
        SELECTORS.DAILY_CHALLENGE_NODE(today, DAILY_CHALLENGES_VARIANTS.TODAY),
      ),
    ).toBeVisible()
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      if (challenge) {
        await expect(
          page.getByTestId(
            SELECTORS.DAILY_CHALLENGE_NODE(
              date,
              DAILY_CHALLENGES_VARIANTS.AVAILABLE,
            ),
          ),
        ).toBeVisible()
      }
    ...
    await expect(
      page.getByTestId(SELECTORS.DAILY_CHALLENGE_RESULT_THUMBNAIL),
...
test.describe("daily challenge 2x2 matrix - playability", () => {
  test("spherical with map: should be playable", async ({ page }) => {
    ...
    await expect(
      page.getByTestId(SELECTORS.DAILY_CHALLENGE_SPHERICAL),
    ).toBeVisible({ timeout: 10_000 })
  ...
  test("flat with map: should be playable", async ({ page }) => {
    ...
    await expect(page.getByTestId(SELECTORS.DAILY_CHALLENGE_FLAT)).toBeVisible({
      timeout: 10_000,
    })
