...
test.describe("invalid user document", () => {
  test("user with missing user doc is logged out and falls back to anonymous", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByTestId("video-create-lobby-button-demo"),
  ...
  test("user with null email in doc is logged out and falls back to anonymous", async ({
    page,
  }) => {
  ...
  test("user with invalid email format in doc is logged out and falls back to anonymous", async ({
    page,
  }) => {
    const { email } = await createAuthUserWithCustomDoc({
      email: "not-an-email",
