...
test.describe("Race playing", () => {
  test("should play a race until the end", async ({ page }) => {
    ...
    await expect(page.getByTestId(SELECTORS.RACE_SCORE)).toHaveText("100 pts", {
      timeout: 10_000,
    })

    // -- Round 2: wrong answer → score should be 50 (100 - 50) --
    await typeAndSelectGame(page, games[2].title) // wrong game for round index 1
    await expect(page.getByTestId(SELECTORS.RACE_SCORE)).toHaveText("50 pts", {
      timeout: 10_000,
    })

    // -- Round 3: correct answer → score should be 150 --
    await typeAndSelectGame(page, games[2].title)
    await expect(page.getByTestId(SELECTORS.RACE_SCORE)).toHaveText("150 pts", {
      timeout: 10_000,
    })
    ...
    await expect(page.getByTestId(SELECTORS.RACE_SCORE)).toHaveText("200 pts", {
      timeout: 10_000,
    })

    // -- Wait for the timer to expire and race to finish --
    await expect(page.getByTestId(SELECTORS.RACE_FINISHED)).toBeVisible({
      timeout: RACE_DURATION * 1000,
    })

    // -- Verify the finished screen shows the score and rounds --
    await expect(
      page.getByTestId(SELECTORS.RACE_FINISHED_SCORE(user.id)),
    ).toHaveText("200 pts")
    await expect(
      page.getByTestId(SELECTORS.RACE_FINISHED_ROUNDS(user.id)),
    ).toHaveText("5 rounds")
