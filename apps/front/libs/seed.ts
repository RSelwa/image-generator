...
import {
  ...
  readyImageItemToRound,
  readyImageItemToSpecialOption,
  type Round,
  roundSchema,
} from "@repo/schemas"
...
export const generateSeedRounds = async ({
  numberOfRounds,
  hasSpecialRounds,
  ...
  try {
    ...
    const metaSnap = await refs[TABLES.METADATA]
    ...
    const formattedSphericalsForNormalRounds = sphericals
      .map(readyImageItemToRound)
      .filter((round) => round !== null)
    const formattedFlatsForNormalRounds = flats
      .map(readyImageItemToRound)
      .filter((round) => round !== null)
    const formattedSphericalsForSpecialRounds = hasSpecialRounds
      ? sphericals
          .map(readyImageItemToSpecialOption)
          .filter((option) => option !== null)
      : []
    const formattedFlatsForSpecialRounds = hasSpecialRounds
      ? flats
          .map(readyImageItemToSpecialOption)
          .filter((option) => option !== null)
      : []
    ...
    for (let index = 0; index < numberOfRounds; index++) {
      ...
      if (isRoundSpecial) {
        ...
        for (let i = 0; i < SPECIAL_ROUND_OPTIONS_COUNT; i++) {
          ...
          const allExcludedIds = [
            ...new Set([...excludedGameIds, ...excludedOptionIds]),
          ]

          const allSpecialOptions = [
            ...formattedSphericalsForSpecialRounds,
            ...formattedFlatsForSpecialRounds,
          ]
          ...
          const sphericalOptions = formattedSphericalsForSpecialRounds.filter(
            (option) =>
              option &&
              !allExcludedIds.includes(option.gameId) &&
              !recentlyPlayedGameIds.includes(option.gameId),
          )
          ...
          const specialRoundOptions =
            sphericalOptions.length > 0 ? sphericalOptions : allFilteredOptions

          let randomOption

          if (specialRoundOptions.length === 0) {
            // Fallback: drop recently played exclusion, but keep current seed exclusion
            const sphericalFallback =
              formattedSphericalsForSpecialRounds.filter(
            ...
            const fallbackOptions =
              sphericalFallback.length > 0 ? sphericalFallback : allFallback

            if (fallbackOptions.length === 0) {
              console.warn(
                "No more suitable options found for special round generation.",
              )
            ...
            randomOption =
              specialRoundOptions[
                Math.floor(Math.random() * specialRoundOptions.length)
              ]
          }
      ...
      const allNormalRounds = [
        ...formattedSphericalsForNormalRounds,
        ...formattedFlatsForNormalRounds,
      ]
      const normalRoundsFiltered = allNormalRounds.filter(
        (round) =>
          round.gameId &&
          !excludedGameIds.includes(round.gameId) &&
          !recentlyPlayedGameIds.includes(round.gameId),
      )
      ...
      if (normalRoundsFiltered.length === 0) {
        ...
        const fallbackFiltered = allNormalRounds.filter(
          (round) => round.gameId && !excludedGameIds.includes(round.gameId),
        )
        ...
        randomRound =
          normalRoundsFiltered[
            Math.floor(Math.random() * normalRoundsFiltered.length)
          ]
      }
