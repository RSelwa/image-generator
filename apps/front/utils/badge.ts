...
export const getBadgeVariantSocials = (
  status: ConstantValues<typeof SOCIALS_STATUS> | null,
) => {
  ...
  if (
    status === SOCIALS_STATUS.IN_PROGRESS_AUDIO_EXTRACTION ||
...
export const getBadgeVariantSounds = (
  status: ConstantValues<typeof SOUND_STATUS> | null,
) => {
...
export const getBadgeTextSounds = (
  status: ConstantValues<typeof SOUND_STATUS> | null,
) => {
  if (status === SOUND_STATUS.DRAFT) return "Draft"
  if (status === SOUND_STATUS.WAITING_FOR_EXTRACTION)
...
export const getBadgeVariantBoolean = (value: boolean) =>
  value ? BADGE_VARIANTS.GREEN : BADGE_VARIANTS.RED

export const getBadgeTextBoolean = (value: boolean) =>
  value ? "True" : "False"
...
export const getBadgeVariantLobbyStatus = (
  status: ConstantValues<typeof LOBBY_STATUS>,
) => {
