...
const RowPlayer = ({
  ...
  return (
    <div className="flex items-center gap-3 py-2 border-b border-neutral-800 last:border-0">
      <span className="font-shapiro-wide text-sm w-5 text-center text-muted-foreground">
        #{rank}
      </span>
      <UserAvatar
        size="sm"
      ...
      />
      <span
        data-text-glow={isTextGlow(donorTier)}
  ...
  return (
    <section className="grid grid-cols-2 lg:grid-cols-4">
      <article className="p-5 border-r border-neutral-900">
        <p className="font-interference uppercase mb-2 text-sm opacity-60">
          {t("leaderboard")}
        </p>
        <h2 className="font-shapiro-wide text-2xl lg:text-3xl mb-6">
          {t("dailyChallengeStreak")}
        </h2>
        <div className="overflow-y-scroll max-h-40">
          {streakPlayers?.map((player, i) => (
            <RowPlayer
              score={player.maxStreak}
              key={player.id}
              player={player}
              rank={i + 1}
        ...
        <div className="overflow-y-scroll max-h-40">
          {deathRunPlayers?.map((player, i) => (
            <RowPlayer
              ...
              player={player}
              rank={i + 1}
              score={player.bestDeathRunScore}
            />
          ))}
        </div>
      </article>
