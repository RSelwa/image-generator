...
import {
  ...
  DailyChallengeLabel,
} from "@/components/daily-challenge/node-icons"
...
export const nodeVariants = cva(
  "flex size-16 overflow-hidden items-center font-mono font-bold justify-center transition-transform",
  {
    variants: {
      variant: {
        [DAILY_CHALLENGES_VARIANTS.FUTURE]:
          "bg-marathon-orange text-marathon-orange-foreground",
        ...
        [DAILY_CHALLENGES_VARIANTS.AVAILABLE]:
          "bg-marathon-yellow text-marathon-yellow-foreground",
...
export const PathNode = memo(
  ({ date, index }: { date: string; index: number }) => {
    ...
    const { data: challenge, isLoading } =
      useGetDailyChallengeEntityByDateQuery({ date })
