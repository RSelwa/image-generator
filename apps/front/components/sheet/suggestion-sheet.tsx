...
export const SuggestionSheet = () => {
  const [suggestionId, setSuggestionId] = useQueryState(
    QUERY_PARAMS.SUGGESTION_ID,
  )
  const adminId = useAppSelector(selectUserId)

  const { data: suggestion } = useGetSuggestionByIdQuery(
    { id: suggestionId || "" },
    { skip: !suggestionId },
  )
  const { data: userSuggestion } = useGetUserByIdQuery(
    { id: suggestion?.createdBy || "" },
    { skip: !suggestion?.createdBy },
  )

  const [findOrCreateConversation] = useFindOrCreateConversationMutation()
  const [sendConversationMessage, { isLoading: isLoadingSendingMessage }] =
    useSendConversationMessageMutation()
  ...
  if (!suggestion)
    return (
      <Sheet
        open={open}
        onOpenChange={(open) => !open && setSuggestionId(null)}
      >
        <EmptySheet />
      </Sheet>
    )
      ...
      <SheetContent>
        ...
        {userSuggestion && (
          <section className="px-4 font-shapiro ">
            <UserAvatar
              {...userSuggestion}
              avatar={userSuggestion?.avatar || ""}
              name={userSuggestion?.pseudo || "?"}
            />
            <p>{userSuggestion?.email}</p>
            <form
            ...
            >
              ...
              <Button
                type="submit"
                disabled={isLoadingSendingMessage}
                variant="outline"
              >
        ...
        <section className="grid flex-1 lg:grid-cols-3 grid-cols-1 auto-rows-min gap-6 px-4">
          {hasImages &&
            suggestion.imagesUrls?.map((url, index) => (
              <Image
                key={url}
                src={url}
                alt={`Suggestion image ${index + 1}`}
                width={200}
                height={200}
                className="rounded"
              />
        ...
        <SheetFooter>
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="marathon-destructive">Delete suggestion</Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>
