...
const DeathRunPlaying = ({
  ...
  const handleGiveUp = () => {
    ...
    finishRun({ deathRunId: deathRun.id, uid }).catch(() =>
      console.error("Failed to finish death run"),
    )
  }
  ...
  const handleWatchAd = () => {
    ...
    showAd({
      ...
      onRewarded: () => {
        revive({ deathRunId: deathRun.id, uid }).catch(() =>
  ...
  useEffect(() => {
    ...
    hasFinished.current = true
    finishRun({ deathRunId: deathRun.id, uid }).catch(() =>
      console.error("Failed to finish death run on lives empty"),
    )
    ...
    <div className="h-full-height flex flex-col">
      <div className="flex items-center justify-between px-6 py-3 border-b bg-background/80 backdrop-blur">
        <span className="text-sm text-muted-foreground">
          Round{" "}
          <span
            data-testid={SELECTORS.DEATH_RUN_ROUND_INDEX}
        ...
        <div
          data-testid={SELECTORS.DEATH_RUN_LIVES}
        ...
        >
          {Array.from({ length: deathRun.lives }).map((_, i) =>
            i < run.livesRemaining ? (
              ...
              <HeartOff key={i} className="size-5 text-muted-foreground" />
            ),
          )}
        </div>

        <span
          data-testid={SELECTORS.DEATH_RUN_SCORE}
        ...
        {isOutOfLives && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 backdrop-blur gap-4">
            ...
            <p className="text-muted-foreground text-sm">
              {run.score} pts — {run.answers.filter((a) => a.isCorrect).length}{" "}
              correct
            </p>
            ...
            {canRevive && !isWatchingAd && (
              <div className="flex flex-col items-center gap-2 mt-2">
                ...
                <Button
                  data-testid={SELECTORS.DEATH_RUN_GIVE_UP}
                  variant="ghost"
                  size="sm"
