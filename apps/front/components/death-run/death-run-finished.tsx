...
const DeathRunFinished = ({ deathRun }: { deathRun: DeathRunDocWithId }) => {
  ...
  const { data: runs = [] } = useGetDeathRunRunsQuery({
    deathRunId: deathRun.id,
  })
  const { data: seed } = useSubscribeMarathonSeedQuery(
    deathRun.seedId ? { seedId: deathRun.seedId } : skipToken,
  )
  ...
  return (
    <div
      data-testid={SELECTORS.DEATH_RUN_FINISHED}
