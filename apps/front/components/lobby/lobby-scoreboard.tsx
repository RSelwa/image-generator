    ...
    <div className="flex flex-col gap-2 w-full max-w-md">
      {sortedPlayers.map((player, index) => (
        <div
          ...
          style={{
            animationDelay: `${(allPlayersLength - index) * 1000}ms`,
            animationFillMode: "both",
            animationDuration: "400ms",
          }}
        >
          <span className="text-muted-foreground font-shapiro-wide font-bold w-6 text-center">
            #{index + 1}
          </span>
          <UserAvatar
            size="sm"
          ...
          />
          <span className="font-medium flex-1 truncate font-mono">
            {player.name}
          </span>
