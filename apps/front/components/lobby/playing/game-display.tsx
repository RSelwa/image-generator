...
const NextRoundButton = (props: ComponentProps<"button">) => {
  ...
  const isEveryoneReady = useAppSelector(
    selectAllPlayersReady(lobbyId, roundIndex),
  )
  ...
  return (
    <Popover open={popOverOpen} onOpenChange={setPopOverOpen}>
      ...
      <PopoverContent
        sideOffset={12}
        className="text-foreground bg-background text-center flex flex-col items-center justify-center gap-6 w-fit font-mono"
      >
        <span className="text-sm text-muted-primary-foreground">
          Not all players have finished the round. <br /> Are you sure to go
          next round ? (they will loose their points on this round)
        </span>
        <div className="flex gap-2 items-center justify-center">
          <Button
            variant="marathon-link"
            data-testid="next-round-button-confirm"
            onClick={() => nextRound({ lobbyId })}
          >
            Yes, go next round
          </Button>
...
const InfoRoundSpecial = () => {
  ...
  return (
    <>
      <ImageGlow>
        <Image
          data-testid={`game-thumbnail-${currentRoundInfos?.gameTitle}`}
          src={currentRoundInfos?.gameThumbnailUrl || FALL_BACK_IMAGE}
          height={300}
          width={300}
          alt={currentRoundInfos?.gameTitle || ""}
          className="max-h-96"
        />
      </ImageGlow>
      <TextRevealTW
        text={`Game guessed: +${currentAnswer?.gamePoints}pts`}
...
const InfosRoundNormal = () => {
  ...
  const percentagePoints = currentRoundData?.pointsDistance
    ? (animatedPoints / (currentRoundData.pointsDistance || 1)) * 100
    : 0
    ...
    <div className="flex flex-col items-center justify-center gap-2">
      ...
      <div className="text-foreground text-lg text-shadow flex items-center gap-2 transition-opacity duration-500 delay-[2s]">
        ...
        <Progress
          ...
          className="bg-neutral-500/50 w-52"
        />
        <span>{currentRoundData.pointsDistance}</span>
      </div>

      <TextRevealTW
        initialDelay={4}
        text={`Total: +${currentAnswer?.points} Pts`}
        className="text-foreground text-lg"
      />
    </div>
  )
  ...
  return (
    <section className="h-full-height absolute z-10 bg-background/90 w-full font-mono py-6">
      <div className="flex flex-col lg:gap-8 gap-8 items-center size-full">
        <Rounds
          currentRound={lobby?.currentRound || 0}
          numberOfRounds={lobby?.config?.numberOfRounds || 0}
        />
        <TextRevealTW
          text={currentRoundInfos?.gameTitle || "Game title"}
          className="text-foreground font-bold font-shapiro-wide text-2xl"
        />
