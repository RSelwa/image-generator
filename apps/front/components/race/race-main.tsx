...
const RaceMain = () => {
  ...
  const { data: race, isLoading: raceLoading } = useSubscribeRaceQuery(
    { raceId },
    { skip: !raceId },
  )
  ...
  if (!race)
    return (
      <p className="p-8 text-center text-muted-foreground">Race not found.</p>
    )

  if (race.status === RACE_STATUS.WAITING) return <RaceWaiting race={race} />

  if (race.status === RACE_STATUS.STARTING)
    return (
