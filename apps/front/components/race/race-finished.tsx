...
export const GameRow = ({ round }: { round: MarathonSeedRound }) => {
  ...
  const { data: gameData, isLoading } = useGetGameByIdQuery(
    { id: round.gameId },
    { skip: !round.gameId },
  )
  ...
  return (
    <Popover>
      <PopoverTrigger>
        <article className="flex relative items-center justify-center py-4">
          <img
            src={round.sphericalImageUrl || round.flatImageUrl || ""}
            alt={game?.title || "Game image"}
            className="size-full absolute opacity-30 inset-0 -z-10 object-cover rounded"
          />
      ...
      <PopoverContent side="right" asChild>
        <div className="w-full flex flex-col items-center  gap-4">
          <img
            src={gameData.image}
            alt={gameData?.title}
            className="size-full max-h-36 object-contain rounded"
          />
...
const RaceFinished = ({ race }: { race: RaceDocWithId }) => {
  ...
  const { data: seed } = useSubscribeMarathonSeedQuery(
    race.seedId ? { seedId: race.seedId } : skipToken,
  )
  ...
  return (
    <div
      data-testid="race-finished"
      className="h-full-height flex flex-col items-center justify-center gap-8 p-6"
    >
        ...
        {runs.map((run, i) => {
          ...
          return (
            ...
            >
              ...
              <span
                data-testid={`race-finished-score-${run.uid}`}
                className="font-mono font-bold text-primary"
              >
                {run.score} pts
              </span>
              <span
                data-testid={`race-finished-rounds-${run.uid}`}
                className="text-xs text-muted-foreground"
              >
                {run.answers.length} rounds
              </span>
            </div>
          )
        })}
      ...
      {seed?.rounds && (
        <section className="w-full max-w-sm space-y-4 border border-primary">
          <h2 className="text-lg font-semibold px-4 font-interference text-primary">
            {t("recap")}
          </h2>
