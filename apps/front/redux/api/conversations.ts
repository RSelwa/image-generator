...
import {
  ...
  type ConversationDocWithId,
  conversationDocWithIdSchema,
  type ConversationMessageDoc,
  type ConversationMessageDocWithId,
  conversationMessageDocWithIdSchema,
} from "@repo/schemas"
import {
  getConversationRef,
  getLobbyConversationId,
  TABLE_REFS,
  TABLES_SUB_REFS,
...
export const conversationsApi = createApi({
  ...
  endpoints: (builder) => ({
      ...
      queryFn: async ({ uid, otherUid }) => {
        try {
          ...
          const existing = snapshot.docs.find((docSnap) =>
            isSameConversation(docSnap.data(), participants),
          )
    ...
    findOrCreateLobbyConversation: builder.mutation<
      ConversationDocWithId,
      { lobbyId: string; adminUid: string }
    >({
    ...
    subscribeConversation: builder.query<
      ConversationDocWithId | null,
    ...
    subscribeConversations: builder.query<
      ConversationDocWithId[],
      { uid: string }
    >({
      queryFn: () => ({ data: [] }),
    ...
    subscribeConversationMessages: builder.query<
      ConversationMessageDocWithId[],
      { conversationId: string }
    >({
      queryFn: () => ({ data: [] }),
        ...
        try {
          ...
          unsubscribe = onSnapshot(
            ...
            (snapshot) => {
              ...
              for (const docSnap of snapshot.docs) {
                const { data, error } =
                  conversationMessageDocWithIdSchema.safeParse({
                    id: docSnap.id,
                    ...docSnap.data(),
                  })
            ...
            (error) => {
              console.error(
                "Error in conversation messages snapshot listener:",
                error,
              )
            },
          )
          ...
          console.error(
            "Error setting up conversation messages listener:",
            error,
          )
        }
