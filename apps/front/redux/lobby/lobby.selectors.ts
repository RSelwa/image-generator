...
export const selectPlayerMyself = memoizeByArgs((lobbyId: string) =>
  createSelector(selectLobby(lobbyId), selectUserId, (lobby) => {
...
export const selectLobbyPlayers = memoizeByArgs((lobbyId: string) =>
  createSelector(
    selectLobby(lobbyId),
    (lobby) => lobby?.players || EMPTY_PLAYERS,
  ),
)

// Current player's round answer
const selectRoundAnswerResult = memoizeByArgs(
  (lobbyId: string, roundIndex: number) =>
    lobbyApi.endpoints.listenRoundAnswer.select({ lobbyId, roundIndex }),
)

export const selectCurrentPlayerRoundAnswer = memoizeByArgs(
  (lobbyId: string, roundIndex: number) =>
    createSelector(selectRoundAnswerResult(lobbyId, roundIndex), (result) => {
...
export const selectHasSelectedOption = memoizeByArgs(
  (lobbyId: string, roundIndex: number) =>
    createSelector(
...
export const selectSelectedOption = memoizeByArgs(
  (lobbyId: string, roundIndex: number) =>
    createSelector(
      selectCurrentPlayerRoundAnswer(lobbyId, roundIndex),
...
export const selectAllPlayersReady = memoizeByArgs(
  (lobbyId: string, roundIndex: number) =>
    createSelector(
      ...
      (result, players) => {
        ...
        return players.every((p) =>
          answers.some((a) => a.uid === p.uid && a.isReadyForNextRound),
        )
      },
    ),
)
...
export const selectMyLivesRemaining = memoizeByArgs(
  (lobbyId: string, roundIndex: number) =>
    createSelector(
...
export const selectIsPlayerEliminated = memoizeByArgs(
  (lobbyId: string, roundIndex: number) =>
    createSelector(
...
export const selectCurrentRoundGameTitle = memoizeByArgs(
  (lobbyId: string, roundIndex: number) =>
    createSelector(
      selectCurrentRoundData(lobbyId),
      selectSelectedOption(lobbyId, roundIndex),
      (roundData, selectedOption) =>
        selectedOption?.gameTitle || roundData?.gameTitle || "",
    ),
)

export const selectCurrentRoundInfos = memoizeByArgs(
  (lobbyId: string, roundIndex: number) =>
    createSelector(
      selectLobby(lobbyId),
      selectSelectedOption(lobbyId, roundIndex),
      (lobby, selectedOption) =>
        lobby?.currentRoundData?.isSpecial
          ? selectedOption
          : lobby?.currentRoundData,
    ),
)
