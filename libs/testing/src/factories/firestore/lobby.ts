...
export const lobbyFactory: FactoryDoc<LobbyDoc & { id: string }> = (
  item = {},
) => {
  const {
    players: itemPlayers,
    playersIds: itemPlayersIds,
    isDemo: itemIsDemo,
    ...rest
  } = item
  const players = itemPlayers || []
  const playersIds = itemPlayersIds || players.map((p) => p.uid)
  const isDemo = itemIsDemo || false
